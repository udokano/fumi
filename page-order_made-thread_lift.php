<?php

/*
 Template Name: オーダーメイドスレッドリフト
 */

?>
<?php get_header(); ?>
<div class="page-head">
  <h1 class="tc f-ryu order__tit">オーダーメイドスレッドリフト</h1>
  <?php echo breadcrumb_func(); ?> </div>
<section class="ope-top-desc order-made-top-desc gosic">
  <p class="tc">溶ける糸を使用してたるみを引き上げる<br class="sp">"切らないフォイスリフト治療"です。<br>スレッド治療で大切なのは、その方の脂肪の<br class="sp">つき方やたるみの状態を見極め、<br class="pc">最適な層の<br class="sp">最適な部位と方向にスレッドを挿入することです。<br>当院のスレッド治療は全て形成外科医の院長が行います。<br>たるみの状態やお悩みに合わせて最適な糸をご提案。<br>数種類の糸を組み合わせて<br class="sp">リフトアップを行うこともあります。</p>
<div class="ancor-box order">
   <a href="#area1"><span class="row__center">スレッド治療の良さ</span></a>
   <a href="#area2"><span class="row__center">糸の種類</span></a>
  <a href="#area3" class="center"><span class="row__center">
	  スレッドリフトと<br>
	  組み合わせが<br>
	  おすすめの治療</span>
	</a>
	<a href="#area4"><span class="row__center">スレッド治療の流れ</span></a>
</div>
  <!-- ./anchor-box -->
</section>
<section>
<div class="order__made__tit bg__p02">
  <h2 class="tc">こんな方におすすめです</h2>
</div>


<div class="recommended">
  <div class="inner">
	<div class="recommended__box flex">
  <div class="recommended__thumb pc-dis">
	<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended.thumb.jpg" alt="先山文">
  </div>
  <ul class="recommended__list">
	<li class="recommended__list__item">フェイスラインのたるみが気になる方</li>
	<li class="recommended__list__item">首のたるみが気になる方</li>
	<li class="recommended__list__item">ほうれい線やマリオネットライン（ブルドックライン）を改善したい方</li>
	  <li class="recommended__list__item">マシン系の治療で効果を感じなかった方</li>
		<li class="recommended__list__item">ぼやけた輪郭を引き締めたい方</li>
		  <li class="recommended__list__item">肌のハリもリフトアップもしたい方</li>
  </ul>
	</div>
  </div>
</div>
</section>
<section>
<div class="order__made__tit bg__p02" id="area1">
  <h2 class="tc">スレッド治療の良さ</h2>
</div>

<div class="quality">
  <div class="inner">
	<ul class="quality__list quality__list__top flex">
	  <li class="quality__list__item quality__list__top__item">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
			高いリフトアップ<br>効果と持続性
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		  気になる箇所にダイレクトにアプローチできるため、満足度の高い治療です。<br>またリフトアップ効果は１年から2年と長期間持続します。<br>（糸の種類によって異なります）
		</p>
	  </li>
	   <li class="quality__list__item quality__list__item--center">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
			安心と安全
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		 糸は通常の手術で使用される吸収糸と同じ素材で作られています。<br>異物として皮下に残り続けることはないためとても安心です。施術後もレントゲンにうつる心配もありません。<br>溶けない糸を使用したリフトアップ治療でも、効果は永久に続くわけではなく継続的な治療を要します。<br>そういった意味でも吸収性の糸でリフトアップしていくことをおすすめしています。</p>
	  </li>
	  <li class="quality__list__item quality__list__top__item">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
		   ダウンタイムが<br>少ない
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		 施術後は麻酔の腫れ感を感じることもありますが、通常通りの生活が可能です。<br>内出血や一時的な凹凸、浮腫みなどを感じることもありますが、数日から1週間程度で落ち着きます。</p>
	  </li>
	</ul>

	<ul class="quality__list quality__list__bottom flex">
	  <li class="quality__list__item quality__list__bottom__item">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
			自然な仕上がり
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		  皮膚を切開して行うフェイスリフトや糸を筋膜に固定する引き上げ治療ではないため、ひきつったお顔になることはなく仕上がりはとても自然です。
		</p>
	  </li>
	   <li class="quality__list__item quality__list__bottom__item">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
		   直後から<br class="sp">効果を実感
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		リフトアップ効果は治療直後から実感していただけます。</p>
	  </li>
	  <li class="quality__list__item quality__list__bottom__item">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
		   美肌効果
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		 糸周囲でコラーゲンが増生され、美肌効果や皮膚の引き締め効果を期待できます。</p>
	  </li>
	  <li class="quality__list__item quality__list__bottom__item">
		<div class="quality__list__sq">
		  <span class="quality__list__sq__tit">
		   傷が残らない
		  </span>
		</div><!--  ./quality__list__sq -->
		<p class="quality__list__text">
		メスを使わないリフトアップ治療になります。<br>糸挿入部位の小さな傷や一時的な凹みを感じることがありますが、数日から１週間程度で目立たなくなります。
		</p>
	  </li>
	</ul>

  </div>
  <!-- ./inner -->
</div>
</section>
<section>
<div class="order__made__tit bg__p02" id="area2">
  <h2 class="tc">糸の種類</h2>
</div>

<div class="lift_type">
  <div class="inner">

  <ul class="lift_type__anchor flex">
	<li class="lift_type__anchor__item">
	  <a href="#lift_type01" class="lift_type__anchor__link">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/lift_ancher_img01.png" alt="シルエットソフト">
	  </a>
	</li>
	 <li class="lift_type__anchor__item">
	  <a href="#lift_type02" class="lift_type__anchor__link">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/lift_ancher_img02.png" alt="シルエットソフト">
	  </a>
	</li>
	 <li class="lift_type__anchor__item">
	  <a href="#lift_type03" class="lift_type__anchor__link">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/lift_ancher_img03.png" alt="シルエットソフト">
	  </a>
	</li>
	 <li class="lift_type__anchor__item">
	  <a href="#lift_type04" class="lift_type__anchor__link">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/lift_ancher_img04.png" alt="シルエットソフト">
	  </a>
	</li>
	 <li class="lift_type__anchor__item">
	  <a href="#lift_type05" class="lift_type__anchor__link">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/lift_ancher_img05.png" alt="シルエットソフト">
	  </a>
	</li>
	 <li class="lift_type__anchor__item">
	  <a href="#lift_type06" class="lift_type__anchor__link">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/lift_ancher_img06.png" alt="シルエットソフト">
	  </a>
	</li>
  </ul>
  </div>
  <!-- ./inner -->
</div>
<!-- ./lift_type -->

<section>
<div class="lift_desc lift_desc__deep box-size">
  <div class="inner">
<h3 class="lift_desc__tit bg__deep__tit">深めに入れるスレッド</h3>
<div class="lift_desc__box bg__deep flex">
  <div class="lift_desc__box__left">
	<div class="lift_desc__box__left__text">
	  6本〜12本程度を使用
	</div>
	<!-- ./lift_desc__box__left__text -->
   <!--  <p class="lift_desc__box__cach">主な効果</p> -->
	<ul class="lift_desc__point flex">
		<li class="lift_desc__point__out bg__deep__cach">
		  <div class="lift_desc__point__inner">
			<p class="lift_desc__point__text cw">1~2年<br>持続効果</p>
		  </div>
		</li>
		<!-- ./lift_desc__point__out -->
		<li class="lift_desc__point__out bg__deep__cach">
		   <div class="lift_desc__point__inner">
			 <p class="lift_desc__point__text cw">高い<br>リフトアップ<br>効果</p>
		   </div>
		</li>
		<!-- ./lift_desc__point__out -->
		<li class="lift_desc__point__out bg__deep__cach">
		   <div class="lift_desc__point__inner">
			 <p class="lift_desc__point__text cw">コラーゲン<br>増生による<br>美肌</p>
		  </div>
		</li>
		<!-- ./lift_desc__point__out -->
	</ul>

  </div>
  <!-- ./lift_desc__box -->
  <div class="lift_desc__box__center">
<div class="lift_desc__box__center__thumb">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/deep_lift_thumb.png" alt="サムネイルイラスト">
</div>
<!-- ./lift_desc__box__center__thumb -->
  </div>
  <div class="lift_desc__box__right">
	<p class="lift_desc__box__cach lift_desc__box__cach--right">糸の働き</p>
	<div class="lift_desc__box__right__ilust">
	  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/deep_lift_illust.png" alt="イラスト">
	</div>
	<!-- ./lift_desc__box__right__ilust -->
  <ul class="lift_desc__box__list">
	<li class="lift_desc__box__list__item">・糸の力で引き上げ効果が期待できます。</li>
	 <li class="lift_desc__box__list__item">・糸の周囲にコラーゲンが生成され保持力が持続します。</li>
	  <li class="lift_desc__box__list__item">・糸の種類によりますが、効果は1〜2年程続きます。</li>
  </ul>
  </div>
  <!-- ./lift_desc__box__right -->
</div>
  <!-- ./lift_desc -->
  </div>
  <!-- ./inner -->

<div class="lift_ope">
	<div class="inner">
	<section>
		  <h4 class="lift_ope__tit border__deep" id="lift_type01">シルエットソフト・<br class="sp">アンカーDXダブル(3Dリフト)</h4>
		  <div class="lift_ope__box flex">
			<div class="lift_ope__box__left">
			  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/deep_lift_item_thumb01.png?0513" alt="イラスト">
			</div>
			<!-- ./lift_ope__box__left -->
			<div class="lift_ope__box__right">
				<p class="lift_ope__box__right__text">
				バイオコーンといわれる円錐状のコーンが付いている糸です。<br class="pc">この円錐状のコーンにより立体的な引き上げが可能となりリフトアップ効果の大変高い糸です。<br class="pc">自然でふっくらとした立体的な引き上げが得意なことから『３Dリフト』ともよばれます。<br class="pc">素材はPLLAで、約1年半で吸収されます。</p>
				<ol class="lift_ope__box__right__list">
				  <li class="lift_ope__box__right__item">・マリオネット部分のたるみが気になる方</li>
				  <li class="lift_ope__box__right__item">・こけずにふっくら自然なリフトアップが希望の方</li>
				  <li class="lift_ope__box__right__item">・お顔全体をナチュラルに引き上げたい方</li>
				</ol>
				<div class="case__btn__wrap case__btn__col2 case__btn__wrap__lift">
				  <a href="http://blog.livedoor.jp/dr_sakiyama/archives/cat_1275235.html" target="_blank" class="case__btn btn__left">症例写真はこちら</a>
					 <a href="<?php echo home_url( '/' ); ?>kogao#silhouette_lift__price" class="case__btn">料金はこちら</a>
			   </div>
			</div>

		  </div>
		  <!-- ./lift_ope__box -->
		  </section>
		  <section>
		   <h4 class="lift_ope__tit border__deep" id="lift_type02">Gコグリフト・アンカープラス</h4>
		  <div class="lift_ope__box flex">
			<div class="lift_ope__box__left">
			  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/deep_lift_item_thumb02.png?0513" alt="イラスト">
			</div>
			<!-- ./lift_ope__box__left -->
			<div class="lift_ope__box__right">
				<p class="lift_ope__box__right__text">
			   鉄条網型の特殊なプレスコグ形状の糸です。<br class="pc">刺上の糸と異なり組織と接する面積が広いため組織をしっかりと固定し保持することが可能でリフティング力のとても強い糸です。<br class="pc">またPCL素材のため柔軟性があり、強いリフティング力を持ちながらも違和感なく自然な仕上がりを可能にしています。<br class="pc">約2年で吸収されます。</p>
				<ol class="lift_ope__box__right__list lift_ope__box__right__list--ib02 lift_ope__box__right__list--ib03">
				  <li class="lift_ope__box__right__item">・フェイスラインを上方向にしっかりと引き上げたい方</li>
				  <li class="lift_ope__box__right__item">・お顔全体、上方向への引き上げがしっかりと欲しい方</li>
				</ol>
				<div class="case__btn__wrap case__btn__col2 case__btn__wrap__lift">
				  <a href="http://blog.livedoor.jp/dr_sakiyama/archives/cat_1253021.html" target="_blank" class="case__btn btn__left">症例写真はこちら</a>
				   <a href="<?php echo home_url( '/' ); ?>kogao#gcog_lift__price"  class="case__btn">料金はこちら</a>
			   </div>
			</div>

		  </div>
		  <!-- ./lift_ope__box -->
		  </section>

		  <section>
		   <h4 class="lift_ope__tit border__deep" id="lift_type03">VOVリフトプレミアム</h4>
		  <div class="lift_ope__box flex">
			<div class="lift_ope__box__left">
			  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/deep_lift_item_thumb03.png?0513" alt="イラスト">
			</div>
			<!-- ./lift_ope__box__left -->
			<div class="lift_ope__box__right">
				<p class="lift_ope__box__right__text">
			   プレスコグ形状のモルディング加工の糸です。<br class="pc">コグ先端部に丸みを持たせてあるのが特徴で、このことによって挿入時の痛みや引き上げ時の痛みを最大限出にくくしています。<br class="pc">10㎝のロングタイプ糸と、6㎝のショートタイプ糸があり、挿入部位によって使い分けたり、併用いたします。<br class="pc">素材はPCLで2年ほどかけて吸収されます。</p>
				<ol class="lift_ope__box__right__list lift_ope__box__right__list--ib04">
				  <li class="lift_ope__box__right__item">・ほうれい線をこめかみ方向に引き上げたい方</li>
				  <li class="lift_ope__box__right__item">・マリオネット部分をこめかみ方向に引き上げたい方</li>
				  <li class="lift_ope__box__right__item">・目元や額のたるみが気になる方</li>
					<li class="lift_ope__box__right__item">・あご裏や首、フェイスラインを引き締めたい方</li>
				</ol>
				<div class="case__btn__wrap case__btn__col2 case__btn__wrap__lift">
				  <a href="http://blog.livedoor.jp/dr_sakiyama/archives/cat_1253022.html" target="_blank" class="case__btn btn__left">症例写真はこちら</a>
				  <a href="<?php echo home_url( '/' ); ?>kogao#vov_lift__price"  class="case__btn">料金はこちら</a>
			   </div>
			</div>

		  </div>
		  <!-- ./lift_ope__box -->
		  </section>
		  <section>
			 <h4 class="lift_ope__tit border__deep" id="lift_type04">セルフロック</h4>
		  <div class="lift_ope__box flex">
			<div class="lift_ope__box__left">
			  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/deep_lift_item_thumb04.png?0513" alt="イラスト">
			</div>
			<!-- ./lift_ope__box__left -->
			<div class="lift_ope__box__right">
				<p class="lift_ope__box__right__text">
			   360度らせん状にコグが付いているタイプの糸です。<br class="pc">そのため、従来のPDO素材のスレッドでありながら、皮下内でのスリップやバナナピール現象（挿入後に糸のスレッド部分が裂けてリフト効果がなくなること）が起きにくくリフティング効果の持続が可能となった糸です。<br class="pc">PDO素材ですので、約半年で吸収されます。</p>
				<ol class="lift_ope__box__right__list lift_ope__box__right__list--ib02">
				  <li class="lift_ope__box__right__item">・ほうれい線をこめかみ方向に引き上げたい方</li>
				  <li class="lift_ope__box__right__item">・マリオネット部分をこめかみ方向に引き上げたい方</li>
				</ol>
				<div class="case__btn__wrap case__btn__col2 case__btn__wrap__lift">
				  <a href="http://blog.livedoor.jp/dr_sakiyama/archives/cat_1137469.html" target="_blank" class="case__btn btn__left">症例写真はこちら</a>
				  <a href="<?php echo home_url( '/' ); ?>kogao#selfu_rock__price" " class="case__btn">料金はこちら</a>
			   </div>
			</div>

		  </div>
		  <!-- ./lift_ope__box -->
	</div>
	<!-- ./inner -->
	</section>
</div>
</div>
</section>

<section class="lift_desc lift_desc__deep box-size">
  <div class="inner">
<h3 class="lift_desc__tit bg__shallow__tit">浅めに入れるスレッド</h3>
<div class="lift_desc__box bg__shallow flex">
  <div class="lift_desc__box__left">
	<div class="lift_desc__box__left__text lift_desc__box__left__text--shallow__border">
	  通常50本程度を使用
	</div>
	<!-- ./lift_desc__box__left__text -->
   <!--  <p class="lift_desc__box__cach bg__shallow__cach">主な効果</p> -->
	<ul class="lift_desc__point flex">
		<li class="lift_desc__point__out bg__shallow__cach">
		  <div class="lift_desc__point__inner">
			<p class="lift_desc__point__text c_gray02">肌の引き締め</p>
		  </div>
		</li>
		<!-- ./lift_desc__point__out -->
		<li class="lift_desc__point__out bg__shallow__cach">
		   <div class="lift_desc__point__inner">
			 <p class="lift_desc__point__text c_gray02">小じわの<br>軽減</p>
		   </div>
		</li>
		<!-- ./lift_desc__point__out -->
		<li class="lift_desc__point__out bg__shallow__cach">
		   <div class="lift_desc__point__inner">
			 <p class="lift_desc__point__text c_gray02">コラーゲン<br>増生による<br>美肌</p>
		  </div>
		</li>
		<!-- ./lift_desc__point__out -->
	</ul>
  </div>
  <!-- ./lift_desc__box -->
  <div class="lift_desc__box__center">
<div class="lift_desc__box__center__thumb">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/shallow_lift_thumb.png" alt="サムネイルイラスト">
</div>
<!-- ./lift_desc__box__center__thumb -->
  </div>
  <div class="lift_desc__box__right">
  <p class="lift_desc__box__cach lift_desc__box__cach--right bg__shallow__cach c_gray02">糸の働き</p>
	<div class="lift_desc__box__right__ilust">
	  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/shallow_lift_illust.png" alt="イラスト">
	</div>
	<!-- ./lift_desc__box__right__ilust -->

  <ul class="lift_desc__box__list">
	<li class="lift_desc__box__list__item">・皮膚表面の浅い部分に施術を施すためハリと引き締め、美肌効果が期待できます。</li>
	 <li class="lift_desc__box__list__item">・麻酔クリームのみで施術が可能です。</li>
  </ul>
  </div>
  <!-- ./lift_desc__box__right -->
</div>
  <!-- ./lift_desc -->
  </div>
  <!-- ./inner -->



<div class="lift_ope mb0">
	<div class="inner">
	<section>
		  <h4 class="lift_ope__tit shallow__border" id="lift_type05">ショッピングリフト（ビューティーエナジー）</h4>
		  <div class="lift_ope__box flex">
			<div class="lift_ope__box__left lift_ope__box__left--shallow">
			  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/shallow_lift_item_thumb01.png?0513" alt="イラスト">
			</div>
			<!-- ./lift_ope__box__left -->
			<div class="lift_ope__box__right lift_ope__box__right--shallow">
				<p class="lift_ope__box__right__text">
				髪の毛のような極細の糸で、皮膚の浅い層の引き締めを行います。<br class="pc">
麻酔クリームのみで施術可能なことや、ダウンタイムが少ない手軽さから、“ショッピング感覚で行える”という意味でショッピングリフトと呼ばれています。<br class="pc">素材は引き締め効果がより期待できるPDOで、半年程度で吸収されますが、<br class="pc">その後もコラーゲン増生による引き締め効果や美肌効果は持続します。

</p>
				<ol class="lift_ope__box__right__list lift_ope__box__right__list--ib lift_ope__box__right__list--sp__ib">
				  <li class="lift_ope__box__right__item">・マリオネット部分外側を引締めたい方</li>
				  <li class="lift_ope__box__right__item">・ほうれい線の上のもたつきが気になる方</li>
				  <li class="lift_ope__box__right__item">・フェイスラインを引締めたい方</li>
				</ol>
				<div class="case__btn__wrap case__btn__col2 case__btn__wrap__lift">
				  <a href="http://blog.livedoor.jp/dr_sakiyama/archives/cat_1271997.html" target="_blank" class="case__btn btn__left">症例写真はこちら</a>
				   <a href="<?php echo home_url( '/' ); ?>kogao#shopping_lift__price"  class="case__btn">料金はこちら</a>
			   </div>
			</div>

		  </div>
		  <!-- ./lift_ope__box -->

		  </section>

		  <section>
		   <h4 class="lift_ope__tit shallow__border" id="lift_type06">スレッドアイリフト(Gグロス)</h4>
		  <div class="lift_ope__box flex">
			<div class="lift_ope__box__left lift_ope__box__left--shallow">
			  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/shallow_lift_item_thumb02.png?0513" alt="イラスト">
			</div>
			<!-- ./lift_ope__box__left -->
			<div class="lift_ope__box__right lift_ope__box__right--shallow">
				<p class="lift_ope__box__right__text">
			 ショッピングリフトのように細い糸でありながら、先が丸まっている鈍針構造の鍼(カニューレタイプ)を使用したショートスレッドです。<br class="pc">このため内出血が出やすい目元への施術が可能となりました。<br class="pc">素材はPCLとなり２年程度で吸収されます。</p>
				<ol class="lift_ope__box__right__list lift_ope__box__right__list--ib lift_ope__box__right__list--ib">
				  <li class="lift_ope__box__right__item lift_ope__box__right__item--pl">・目元のたるみが気になる方</li>
				  <li class="lift_ope__box__right__item lift_ope__box__right__item--pl">・目元の小じわが気になる方</li>
				   <li class="lift_ope__box__right__item lift_ope__box__right__item--pl">・目元にハリを出したい方</li>
				  <li class="lift_ope__box__right__item lift_ope__box__right__item--pl">・目元をふっくら若々しくしたい方</li>
				</ol>
				<div class="case__btn__wrap case__btn__col2 case__btn__wrap__lift">
				  <a href="http://blog.livedoor.jp/dr_sakiyama/archives/cat_1271997.html" target="_blank" class="case__btn btn__left">症例写真はこちら</a>
				   <a href="<?php echo home_url( '/' ); ?>kogao#thread_lift__price"  class="case__btn">料金はこちら</a>
			   </div>
			</div>

		  </div>
		  <!-- ./lift_ope__box -->

		  </div>
		  <!-- ./lift_ope__box -->
		  </section>
		  </section>

</div>
</div>
<!-- ./inner -->
<section>
<div class="order__made__tit bg__p02" id="area3">
  <h2 class="tc">スレッドリフトと組み合わせが<br class="sp">お勧めの治療</h2>
</div>

<div class="recommended_thumb">

<ul class="recommended_thumb__list">
  <li class="recommended_thumb__list__item">
	  <a href="<?php echo home_url( '/' ); ?>tarumi#area1">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended_thumb01.png" alt="スレッドリフト×ヒアルロン酸" class="pc">
		<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended_thumb01_sp.png" alt="スレッドリフト×ヒアルロン酸" class="sp">
	  </a>

	  <p class="recommended_thumb__list__desc">
		たるみだけではなく、お顔にかげやコケ感がある方、また左右差がある方にはヒアルロン酸を併用することで左右差が整いボリュームロスも改善されより若々しく美しい仕上がりになります。
	  </p>
  </li>

  <li class="recommended_thumb__list__item">
  <a href="<?php echo home_url( '/' ); ?>kogao#area1">
	  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended_thumb02.png" alt="スレッドリフト×ヒアルロン酸" class="pc">
	  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended_thumb02_sp.png" alt="スレッドリフト×ヒアルロン酸" class="sp">
	   </a>

		<p class="recommended_thumb__list__desc">
		お顔に脂肪のもたつきが多い場合はBNLSneo・カベルライン等の脂肪溶解注射で重さを減らしてからスレッドリフトで引き上げることで、よりリフトアップ効果と持続力が高まります。
	  </p>
  </li>

  <li class="recommended_thumb__list__item">
  <a href="<?php echo home_url( '/' ); ?>siwa#area3">
	  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended_thumb03.png" alt="スレッドリフト×ヒアルロン酸" class="pc">
	  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/recommended_thumb03_sp.png" alt="スレッドリフト×ヒアルロン酸" class="sp">
	   </a>

		<p class="recommended_thumb__list__desc">
		目元の小じわやちりめんジワにはスレッドアイリフトにスネコスの併用、頬などの細かい小じわにはショッピングリフトやシルエットソフトとスネコスを併用することでより効果が高まります。
	  </p>
  </li>
</ul>

</div>
<!-- ./recommended_thumb -->
</section>

<section>

<div class="order__made__tit bg__p02" id="area4">
  <h2 class="tc">スレッド治療の流れ</h2>
</div>

<div class="thread_flow">
  <div class="inner">

  <!-- step01 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step1
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb01.jpg?0513" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">診察</h3>
<p class="thread_flow__contents__text">お顔の状態を診察しお悩みやご希望を伺いながら、適切な糸やデザインを決めていきます。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

 <!-- step02 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step2
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb02.jpg?0513" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">洗顔</h3>
<p class="thread_flow__contents__text">クレンジングと洗顔料でメイクや皮脂などの汚れを落とします。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>


<!-- step03 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step3
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb03.jpg?0430" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">撮影</h3>
<p class="thread_flow__contents__text">施術前の状態を特殊な3Dカメラを用いて撮影をします。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

<!-- step04 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step4
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb04.jpg?0513" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">デザイン</h3>
<p class="thread_flow__contents__text">お顔の状態に合わせてお顔に糸のデザインをしていきます。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

<!-- step05 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step5
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb05.jpg?0513" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">麻酔</h3>
<p class="thread_flow__contents__text">糸の挿入部分に麻酔をしていきます。<br class="pc">
ショッピングリフトやスレッドアイリフトは麻酔クリームのみで施術します。

</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

<!-- step06 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step6
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb06.jpg?0513" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">施術</h3>
<p class="thread_flow__contents__text">糸を挿入し、皮下に留置します。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

<!-- step07 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step7
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb07.jpg" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">クーリング・圧迫</h3>
<p class="thread_flow__contents__text">施術部位を圧迫し冷却することで腫れや内出血を予防します。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

<!-- step08 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step8
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb08.jpg?0430" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
<h3 class="thread_flow__contents__tit">撮影</h3>
<p class="thread_flow__contents__text">施術後の状態を再度3Dカメラを用いて撮影します。<br class="pc">施術前の状態との比較して見ていただくことも可能です。</p>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

<div class="thread_flow__arrow">
  <img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_arrow.png" alt="矢印">
</div>

<!-- step09 -->

<div class="thread_flow__contents box-size">

<div class="thread_flow__contents__left flex">
	<p class="thread_flow__contents__step">
  Step9
</p>
<div class="thread_flow__contents__thumb">
<img src="<?php echo get_template_directory_uri(); ?>/img/operation/order_made_thread_lift/flow_thumb09.jpg" alt="診察">
</div>
<!-- ./thread_flow__contents__thumb -->
  <h3 class="thread_flow__contents__tit">施術終了</h3>
<p class="thread_flow__contents__text">
  スキンケアをしてお帰りいただけます。<br class="pc">糸の種類によっては当日からメイクが可能です。
</p>
</div>
</div>
<!-- ./thread_flow__contents__left -->

</div>
<!-- ./thread_flow__contents -->

 </div>
 <!-- ./inner -->


</section>
<section class="conversion__area conversion__area--bg__none">
  <div class="inner">
	<h2 class="tc gd f-ryu"><span>来院予約・お問合せはこちらから</span></h2>
	<ul class="cv-btn f-ryu">
	  <li class="bg__n_rs">
		  <a href="https://fumibeauty.reserve.ne.jp/" target="_blank">
			<i><img src="<?php echo get_template_directory_uri(); ?>/img/common/icon_online.png" alt="オンライン予約"></i>
			<span>オンライン<br class="sp">予約</span>
		  </a>
	  </li>
	  <li class="bg__n_ct">
		  <a href="<?php home_url( '/' ); ?>contact">
			<i><img src="<?php echo get_template_directory_uri(); ?>/img/common/icon_mail04.png" alt="お問合せ"></i>
			<span>お問合せ</span>
		  </a>
	   </li>
	  <li class="bg__n_line">
		 <a href="https://line.me/R/ti/p/%40304ngjfp" target="_blank">
		  <i><img src="<?php echo get_template_directory_uri(); ?>/img/common/icon_line04.png" alt="LINEで簡単予約"></i>
		  <span>LINEで<br class="sp">簡単予約</span>
		 </a>
	  </li>
	</ul>
  </div>
  <!-- ./inner-box -->
</section>
<?php get_footer(); ?>
